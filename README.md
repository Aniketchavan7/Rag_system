...
- Multi-column tables in the PDF (e.g., Section 3.4's Challenges & Mitigation Strategies table on Page 36) are flattened column-by-column by standard extractors like pypdf rather than row-by-row. This can separate challenge headers from their descriptions across chunk boundaries. A production setup would benefit from a layout-aware parser (such as pdfplumber or Unstructured) to preserve table grid relationships before chunking.
- Front-matter handling: Pages 2–4 (company blurb, standalone title page, preface) are excluded from ingestion via `EXCLUDED_PAGES` in `config.py` because their high keyword density ("Agentic AI") artificially crowded out substantive technical pages during broad retrieval. Page 5 (Table of Contents) and Page 6 (empirical pull quotes) are retained so structural/overview and statistical queries remain fully supported.
